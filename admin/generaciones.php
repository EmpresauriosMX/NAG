              ...
              <div class="card card-danger">
                  ...
                  <div class="card-body">
                    <table class="table">
                      ...
                      <tbody>
                      <?php
                        $ciclos= mysqli_query($link, "SELECT `ciclo`.ID_Ciclo, ciclo.fecha_inicio , ciclo.fecha_fin FROM `generaciones-ciclos`,`ciclo` WHERE  `ciclo`.`ID_Ciclo`= `generaciones-ciclos`.`ID_Ciclo` and `ciclo`.`estatus`= 'activo'");
                        $hay_ciclo_activo = mysqli_num_rows($ciclos);
                        $ciclo_actual=mysqli_fetch_array($ciclos);
                        $cont_ciclos = 0;                                                 
                        //Si existe un ciclo actual en la tabla de los ciclos se crea el primer elemento que es el ese ciclo
                                ...
                                <div class="modal-content">
                                  ...
                                  <div class="modal-footer justify-content-between">
                                    ...
                                    <button type="button" class="btn btn-danger" data-dismiss="modal" onClick=remover_ciclo('.$ciclo_actual["ID_Ciclo"].')>Eliminar</button>
                                          ...
                                          //----CICLOS FUTUROS
                                          $ciclos_futuros = mysqli_query($link, "SELECT `ciclo`.ID_Ciclo, ciclo.fecha_inicio , ciclo.fecha_fin FROM `generaciones-ciclos`,`ciclo` WHERE `ciclo`.`ID_Ciclo`= `generaciones-ciclos`.`ID_Ciclo` and `ciclo`.`estatus`= 'futuro'");
                                          $n_ciclos_futuros = mysqli_num_rows($ciclos_futuros);
                                          //$ciclos_futuros = mysqli_fetch_array($ciclos_futuros);
                                          while($row_ciclos_futuros = mysqli_fetch_array($ciclos_futuros)){
                                            $cont_ciclos += 1;
